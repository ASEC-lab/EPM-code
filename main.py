...
def epm_orig_new_method():
    ...
    formatted_ages = train_ages #format_array_for_enc(train_ages)
    formatted_correlated_meth_val = correlated_meth_val #format_array_for_enc(correlated_meth_val)
    # run the algorithm
    # epm = EPM(correlated_meth_val, train_ages)
...
def test_ckks():
    ...
    #HE.align_mod_n_scale(c_mean, c_mse_1)
    HE.rescale_to_next(c_mean)
    test = c_mse_1 * c_mean
    print("\n5. Rescaling & Mod Switching.")
    print("->\tMean: ", c_mean)
    print("->\tMSE_1: ", c_mse_1)
...
def test_mle():
    csp = CSP()
    mle = MLE(csp)
    Y = np.array([2, 5, 9, 7, 20, 15], dtype=np.double)
    ages = np.array([10, 11, 12], dtype=np.double)
    t = np.array([1], dtype=np.double)
    enc_t = csp.encrypt_array(t)
    #encrypted_Y = csp.encrypt_array(Y)
    #encrypted_ages = csp.encrypt_array(ages)
    all_t = mle.enc_array_same_num1(enc_t, 4097)
    dec_all_t = csp.decrypt_arr(all_t)
